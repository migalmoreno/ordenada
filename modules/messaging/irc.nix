...
mkFeature {
  ...
  options =
    ...
    let
      ...
      inherit (ordenada-lib) mkEnableTrueOption;
    ...
    in
    {
          ...
          types.submodule (
            { name, ... }:
            {
              options = {
                ...
                port = mkOption {
                  type = types.int;
                  default = config.ordenada.features.irc.erc.defaultPort;
      ...
      erc = {
        ...
        sidebarHeaderLineFormat = mkOption {
          type = types.nullOr types.str;
          default =
            if config.ordenada.features.emacs.appearance.headerLineAsModeLine then " ERC Status" else null;
          description = "The header line format for the ERC status side bar.";
        };
        sidebarModeLineFormat = mkOption {
          type = types.nullOr types.str;
          default =
            if config.ordenada.features.emacs.appearance.headerLineAsModeLine then null else "ERC Status";
          description = "The header line format for the ERC status side bar.";
        };
  ...
  homeManager =
    { config, pkgs, ... }:
    {
      programs.emacs =
        ...
        with emacs;
        with irc.erc;
        ordenada-lib.mkElispConfig pkgs {
          name = "ordenada-erc";
          config =
            with ordenada-lib.elisp; # elisp
            ''
              (eval-when-compile
                (require 'erc)
                (require 'xdg)
              ...
              ${lib.optionalString (consult.enable && consult.initialNarrowing) ''
                      ...
                      toString (
                        lib.mapAttrsToList (name: acc: ''
                          (make-ordenada-erc-user
                           ...
                           :nick "${acc.nick}"
                           :tls-p ${toBoolean acc.tls}
                           :client ${toNilOr acc.client ''"${acc.client}"''}
                           :bouncer-p ${toBoolean acc.bouncer})
                        '') config.ordenada.features.irc.accounts
                      )
                    }))
              ...
              (with-eval-after-load 'erc-status-sidebar
                ...
                ${toNilOr sidebarModeLineFormat ''"${sidebarModeLineFormat}"''})
                (setq erc-status-sidebar-header-line-format
                ${toNilOr sidebarHeaderLineFormat ''"${sidebarHeaderLineFormat}"''})
              ...
              (with-eval-after-load 'erc
                ...
                ${lib.optionalString (nick != null) ''(setopt erc-nick "${nick}")''}
                ${lib.optionalString (fullName != null) ''
                  (setopt erc-user-full-name "${fullName}")
                ''}
                (setopt erc-hide-list ${toList hideList})
                (setopt erc-hide-prompt t)
                (setopt erc-hide-timestamps t)
                (setopt erc-echo-timestamps nil)
                ${lib.optionalString killBuffersOnQuit ''
                ...
                (setopt erc-header-line-format ${toNilOr headerLineFormat ''"${headerLineFormat}"''})
                (setopt erc-auto-query ${toNilOr autoQuery "'${autoQuery}"})
                (setopt erc-query-display ${toNilOr queryDisplay "'${queryDisplay}"})
                (setopt erc-join-buffer ${toNilOr joinBuffer "'${joinBuffer}"})
                (setopt erc-timestamp-format "%H:%M")
                (setopt erc-prompt-for-password ${toBoolean promptForPassword})
                ...
                (with-eval-after-load 'erc-track
                  ...
                  (setopt erc-track-exclude-types ${toList trackExcludeTypes}))
                ${
                  if autojoin then
                    ''
                      (with-eval-after-load 'erc-join
                        ...
                        (setopt erc-autojoin-channels-alist ${toAlist autoJoinChannels}))
                    ''
                  else
                    "(erc-autojoin-mode 0)"
                }
                ...
                ${lib.optionalString spelling.enable ''
                  ...
                  (with-eval-after-load 'erc-log
                    ...
                    (setopt erc-log-channels-directory
                            (expand-file-name "emacs/erc-logs"
                                              (xdg-cache-home))))
                ''}
                ${lib.optionalString alignNicknames ''
