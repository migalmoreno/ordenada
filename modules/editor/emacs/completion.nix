...
let
  inherit (pkgs.lib.ordenada) mkHomeConfig mkElispConfig;
in
{
  options.ordenada.features.emacs.completion = {
    enable = lib.mkEnableOption "the Emacs completion feature";
  };
  config.home-manager = mkHomeConfig config "emacs.completion" (user: {
