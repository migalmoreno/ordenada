  ...
  homeManager =
    { config, pkgs, ... }:
    {
      programs.emacs = ordenada-lib.mkElispConfig pkgs {
        ...
        config = with config.ordenada.features; ''
          (eval-when-compile
            (require 'message)
            (require 'sendmail))
          ...
          ${lib.optionalString emacs.gnus.enable ''
          ...
          (with-eval-after-load 'message
            ...
            (setopt message-signature "${mail.defaultMessageSignature}")
          ...
          ${lib.optionalString mail.msmtp.enable ''
            (setopt sendmail-program "${mail.msmtp.package}/bin/msmtp")
          ...
          ''}
          ${
            lib.optionalString (userInfo.gpgPrimaryKey != null) ''
              (setq mml-secure-openpgp-signers '("${userInfo.gpgPrimaryKey}"))
              (add-hook '${lib.optionalString emacs.gnus.enable "gnus-"}message-setup-hook
